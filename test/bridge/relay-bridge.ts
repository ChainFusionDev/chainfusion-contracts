...
describe('RelayBridge', function () {
  ...
  it('should transmit data', async function () {
    ...
    await expect(relayBridge.transmit(fromChainId, data)).to.emit(relayBridge, 'TransmittedData').withArgs(hash);

    await expect(relayBridge.transmit(fromChainId, data)).to.be.revertedWith('RelayBridge: data already transmitted');

    const bridgeUser = await ethers.getContractAt('RelayBridge', relayBridge.address, user);
    await expect(bridgeUser.transmit(fromChainId, data)).to.be.revertedWith('RelayBridge: only validator');
